...
def ema_cross_signals(price: pd.DataFrame, short_span: int, long_span: int) -> pd.DataFrame:
    """
    EMA crossover signals on ``price`` (columns ``date``, ``close``).

    Uses ``ewm(span=..., adjust=False)`` (standard trading EMA).

    - **buy**  : prior bar ``EMA_short <= EMA_long`` and current ``EMA_short > EMA_long``
    - **sell** : prior bar ``EMA_short >= EMA_long`` and current ``EMA_short < EMA_long``
    - **hold** : otherwise

    Bars before ``max(short_span, long_span)`` are **hold** (warm-up, aligned with SMA module).
    """
    if short_span <= 0 or long_span <= 0:
        raise ValueError("EMA spans must be positive.")
    if short_span >= long_span:
        raise ValueError("EMA crossover expects short_span < long_span.")

    df = price.copy()
    c = df["close"].astype(float)
    ema_s = c.ewm(span=short_span, adjust=False).mean()
    ema_l = c.ewm(span=long_span, adjust=False).mean()

    sig: list[str] = []
    start = max(short_span, long_span)
    for i in range(len(df)):
        if i < start:
            sig.append("hold")
            continue
        ps, pl = float(ema_s.iloc[i - 1]), float(ema_l.iloc[i - 1])
        cs, cl = float(ema_s.iloc[i]), float(ema_l.iloc[i])
...
def latest_signal(
    ...
    sig_df = ema_cross_signals(price, short_span, long_span)
...
def load_best_parameters_table() -> pd.DataFrame:
    ...
    Priority:
    1. ``output/brute_ema_cross/best_params_all.json``
    2. ``output/brute_ema_cross/backtest_all_stocks.csv``
    3. ``Algorithms/brute_ema_cross/best_params/*.json``
    """
...
def run_live_screen() -> Path:
    """
    Screen symbols using saved best EMA spans; emit rows where the **latest**
    15m bar is ``buy`` or ``sell``.

    Output: ``output/brute_ema_cross/live_signals.csv``.
    """
    _ensure_root_on_path()
    ...
    if best.empty:
        raise RuntimeError(
            ...
            f"Run: python -m Algorithms.brute_ema_cross.backtest_all"
        )
    ...
    if getattr(settings, "EMAIL_ON_LIVE_SIGNALS", False):
        ...
        mail_live_signals_csv_if_nonempty(
            csv_path=out_path,
            strategy_label="EMA crossover (brute_ema_cross)",
        )
