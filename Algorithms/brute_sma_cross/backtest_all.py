"""
Bulk-download all symbols once, brute-force best SMAs per symbol in parallel,
and write a **single** ``best_params_all.json`` (+ optional summary CSV).

Flow
----
1. Read ``input/STOCKS.csv`` symbol list.
2. ``fetch_indian_equities`` with ``period=max``, ``interval=1d`` (batched ``yf.download``;
   up to ``FETCH_BATCH_PARALLEL_WORKERS`` batches concurrently via ``ProcessPoolExecutor``).
3. ``ProcessPoolExecutor`` runs the SMA grid per symbol on in-memory prices.
4. Saves ``output/brute_sma_cross/best_params_all.json`` (used by ``signals.py``).
5. Saves ``output/brute_sma_cross/backtest_all_stocks.csv`` (flattened merge for spreadsheets).

Usage (from project root)::

    python -m Algorithms.brute_sma_cross.backtest_all
"""
...
from Algorithms.brute_sma_cross.runner import (
